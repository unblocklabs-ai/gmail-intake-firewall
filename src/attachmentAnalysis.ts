...
const MIME_EXTENSION_HINTS: Array<{ pattern: RegExp; extensions: Set<string> }> = [
...
export function analyzeAttachments(attachments: AttachmentMetadata[]): AttachmentRiskMetadata[] {
  return attachments.map(analyzeAttachment);
}

export function analyzeAttachment(attachment: AttachmentMetadata): AttachmentRiskMetadata {
  ...
  const analyzed: AttachmentRiskMetadata = {
    ...attachment,
    riskHints,
...
function doubleExtension(filename: string): boolean {
...
function hasMimeExtensionMismatch(mimeType: string | undefined, extension: string): boolean {
  if (!mimeType) {
    return false;
  }
