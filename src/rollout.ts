...
export function buildRolloutReadiness(input: {
  config: PluginConfig;
  validation: Array<{ severity: "error" | "warning"; path: string; message: string }>;
  status: Record<string, unknown>;
  auth: Record<string, unknown>;
}): RolloutReadiness {
  const findings: RolloutFinding[] = input.validation.map((finding) => ({ ...finding }));
  ...
  const verdict: RolloutVerdict = errorCount > 0
    ? "blocked"
...
export function buildEffectiveActions(config: PluginConfig): RolloutReadiness["actions"] {
  return {
...
function buildActionReadiness(
  config: PluginConfig,
  authSources: Array<Record<string, unknown>>,
  findings: RolloutFinding[],
): RolloutReadiness["actions"] {
  ...
  const actions: RolloutReadiness["actions"] = {
    dryRunOverride: config.dryRun,
    gmail: {
...
function addActionFinding(findings: RolloutFinding[], path: string, action: RolloutAction, label: string): void {
  if (!action.ready) {
    findings.push({
      severity: "error",
      path,
...
function buildSourceReadiness(
  config: PluginConfig,
  statusSources: Array<Record<string, unknown>>,
  authSources: Array<Record<string, unknown>>,
  findings: RolloutFinding[],
...
function buildProductionChecklist(
  config: PluginConfig,
  actions: RolloutReadiness["actions"],
  sources: Array<Record<string, unknown>>,
  findings: RolloutFinding[],
): RolloutReadiness["productionChecklist"] {
  return [
...
): RolloutReadiness["productionChecklist"][number] {
  return {
    id,
...
function arrayOfRecords(value: unknown): Array<Record<string, unknown>> {
  return Array.isArray(value)
    ? value.filter((entry): entry is Record<string, unknown> => Boolean(recordValue(entry)))
    : [];
}
...
function dedupeFindings(findings: RolloutFinding[]): RolloutFinding[] {
