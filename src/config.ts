...
export function validatePluginConfig(config: PluginConfig): ConfigValidationFinding[] {
...
function isHostOnlySecretRef(ref: unknown): boolean {
  const raw = asRecord(ref);
  if (Object.keys(raw).length === 0) {
    return false;
  }
  if (
    stringValue(raw.accessToken)
