...
const MARKETPLACE_PACKAGE_DIR = path.join(ROOT, "marketplace", PLUGIN_ID);
...
if (CHECK_MODE) {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), `${PLUGIN_ID}-marketplace-`));
  try {
    const expectedDir = path.join(tempDir, PLUGIN_ID);
