...
export function analyzeLinks(links, maxDisplayedUrlChars = 160) {
    return links.map((link) => analyzeLink(link, maxDisplayedUrlChars));
}
export function summarizeLinkMetadata(link, maxDisplayedUrlChars = 160) {
    let parsed;
    try {
...
export function analyzeLink(link, maxDisplayedUrlChars = 160) {
    const riskHints = [];
    let parsed;
    try {
        parsed = new URL(link.url);
...
function safeDisplayedUrl(parsed, maxChars) {
